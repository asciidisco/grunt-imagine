...
module.exports = function(grunt) {
    ...
    exports.processImageFiles = function(tools, files, dest, quality, task, done) {
        var toolsToProcessInf = _.compact(_.map(tools, function (tool) {
            ...
            taskSrc = grunt.config.get(task).src,
            processableFiles = [],
            processableTools = [],
        ...
        files.forEach(function (file, idx) {
            var fileOutput = path.join(dest, exports.pathFromSrc(taskSrc, file)),
    ...
    exports.fileType = function (src) {
        var table = {
                '424d': 'image/bmp',
                '50340a': 'image/x-portable-bitmap',
                '50360a': 'image/x-portable-anymap',
                'ffd8ff': 'image/jpeg',
                '47494638': 'image/gif',
                '89504e47': 'image/png',
                '4d4d002a': 'image/tiff',
        ...
        fs.closeSync(fd);
        header = buffer.toString('hex').toLowerCase();
        grunt.log.debug('File: ' + src + ' - Header: ' + header);

        return (match = table[header]) ? match :
            (match = table[header.substring(0,6)]) ? match :
                (match = table[header.substring(0,4)]) ? match :
                    'type/unkown';
    };

    exports.pathFromSrc = function (src, file) {
        ...
        switch (grunt.util.kindOf(src)) {
            case 'string':
                if (grunt.file.isMatch(src, normFile)) {
                    globs.push(path.normalize(src));
                }
                break;

            case 'array':
                src.forEach(function (pattern) {
                    if (grunt.file.isMatch(pattern, normFile)) {
                        globs.push(path.normalize(pattern));
                    }
                });
        }

        // assuming there should always be just one
        // source pattern that matches one source file
        // may need fix if there are more though I'm not sure
        // if it makes sense (in case it'd always write to
        // the first match eventually the wrong one)
        //
        // this hopefully will return the path beginning
        // at the first *dynamic* appearance in the file-path
        // (starting from the path separator) e.g.
        // test/*.jpg -> *.jpg
        // test/**/*.jpg -> /any-dir/*.jpg
        // test/src*/*.jpg -> /src*/*.jpg
        return rgx.test(globs[0]) ? normFile.substring(globs[0].lastIndexOf(path.sep, rgx.exec(globs[0]).index)) : path.basename(file);
    };
